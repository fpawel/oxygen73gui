...
{$ENDREGION 'Internal Declarations'}
    public
        ...
        { Appends a segment of an array of bytes.
          ...
          until the end of the ABytes array. If the index is invalid, nothing
          happens. }
        procedure Append(const ABytes: array of Byte;
        ...
        procedure Append(const ABytes: TBytes;
          const AIndex, ASize: Integer); overload;
...
{ Tries to convert a hex string to a byte array.
  ...
  contains an odd number of characters, then it is prefixed with an
  additional 0. }
function goTryParseHexString(const AString: String; out ABytes: TBytes)
  : Boolean;
...
function host_statistics(host_priv: host_t; flavor: host_flavor_t;
  host_info_out: host_info_t; var host_info_outCnt: mach_msg_type_number_t)
  : kern_return_t; cdecl; external libc name _PU + 'host_statistics';
...
function mallinfo: TMallinfo; cdecl; external libc name _PU + 'mallinfo';
...
begin
    ...
    for I := 0 to Length(Result) - 1 do
        Result[I] := (goCharToHex(S.Chars[(I shl 1)]) shl 4) or
          goCharToHex(S.Chars[(I shl 1) + 1]);
end;
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                // 000uuuuu zzzzyyyy yyxxxxxx => 11110uuu
                Inc(D);
                D^ := ((Codepoint shr 12) and $3F) or $80;
                // 000uuuuu zzzzyyyy yyxxxxxx => 10uuzzzz
                Inc(D);
                D^ := ((Codepoint shr 6) and $3F) or $80;
                // 000uuuuu zzzzyyyy yyxxxxxx => 10yyyyyy
                Inc(D);
                D^ := (Codepoint and $3F) or $80;
                // 000uuuuu zzzzyyyy yyxxxxxx => 10xxxxxx
                Inc(D);
            end
            else
            begin
                ...
                Inc(D);
                D^ := ((Codepoint shr 6) and $3F) or $80;
                // zzzzyyyy yyxxxxxx => 10yyyyyy
                Inc(D);
                D^ := (Codepoint and $3F) or $80;
                // zzzzyyyy yyxxxxxx => 10xxxxxx
                Inc(D);
            end;
        end;
    end;
...
function goUtf8ToUtf16(const ASource: Pointer;
  const ASourceLength: Integer): String;
var
...
begin
    ...
    begin
        ...
        SetLength(Result, goUtf8ToUtf16(ASource, ASourceLength,
          Pointer(Result)));
    end;
end;
...
begin
    ...
    begin
        ...
        begin
            ...
            else if ((Codepoint shr 5) = $06) then
            begin
                { 2-byte sequence (U+80..U+7FF)
                  110yyyyy 10xxxxxx => 00000yyy yyxxxxxx }
                D^ := ((Codepoint shl 6) and $7FF)
                // 110yyyyy => 00000yyy yy000000
            ...
            begin
                ...
                begin
                    ...
                    Codepoint := ((Codepoint shl 12) and $FFFF)
                    // 1110zzzz => zzzz0000 00000000
                ...
                begin
                    ...
                    Codepoint := ((Codepoint shl 18) and $1FFFFF)
                    // 11110uuu => 000uuu00 00000000 00000000
                      + ((S^ shl 12) and $3FFFF);
                    // 10uuzzzz => 000000uu zzzz0000 00000000
                    Inc(S);
                    Inc(Codepoint, (S^ shl 6) and $FFF);
                    // 10yyyyyy => 00000000 0000yyyy yy000000
                    Inc(S);
                    Inc(Codepoint, S^ and $3F);
                    // 10xxxxxx => 00000000 00000000 00xxxxxx
                    Inc(S);
                    Dec(SrcLength, 4);
...
begin
    ...
    if (AIndex < 0) or (ALength < 0) or ((AIndex + ALength) >= Length(ABytes))
    then
...
try
    ...
    on E: Exception do
        Writeln('Grijjy.sysUtils: exception: ' + E.ClassName + ': ' +
          E.Message);
end;
